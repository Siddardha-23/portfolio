"""
AWS Lambda Handler for Chat Microservice
...
Architecture:
    API Gateway (HTTP API) -> Lambda -> apig-wsgi -> Flask App -> Gemini API
"""
...
logger.info(f"INIT phase complete: {_init_duration_ms}ms "
            f"(Flask + chat blueprint + X-Ray SDK)")

# Cold start flag — True only for the first invocation
_cold_start = True

# Persist cold start info for observability
_container_birth = {
