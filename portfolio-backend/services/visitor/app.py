...
def create_app():
    """
    Application factory for the Visitor microservice.

    Registers only visitor-related blueprints:
    - info: Visitor tracking and analytics
    ...
    if os.getenv('USE_SSM_SECRETS', 'false').lower() == 'true':
        try:
            ...
            xray_recorder.configure(service='visitor-service')
    ...
    # Register blueprints - visitor-related only
    ...
    logger.info("Visitor service initialized with info, contact, session, geolocation blueprints")
    return app
