    ...
    return (
        <>
            ...
            >
                <Button
                    onClick={handleOpen}
                    className="h-10 w-10 rounded-full bg-gradient-to-br from-teal-500 to-sky-500 p-0 shadow-lg"
                    title="Visitor map"
            ...
            {createPortal(
                <AnimatePresence>
                    {isOpen && (
                        <>
                            <motion.div
                                className="fixed inset-0 z-[9999] cursor-pointer bg-black/85 backdrop-blur-md"
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                            ...
                            <motion.div
                                ...
                                className="fixed left-1/2 top-1/2 z-[10000] flex -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-[#05070d] shadow-2xl sm:rounded-3xl"
                            ...
                            >
                                ...
                                <div className="flex shrink-0 items-center justify-between border-b border-white/[0.06] px-4 py-4 sm:px-6">
                                    <div className="flex min-w-0 flex-1 items-center gap-3">
                                        <div className="shrink-0 rounded-xl bg-gradient-to-br from-teal-500 to-sky-500 p-2.5 shadow-lg shadow-teal-500/20">
                                            <Globe className="h-5 w-5 text-white" />
                                        </div>
                                        <div className="min-w-0">
                                            <h2 className="font-semibold text-white">Global Visitors</h2>
                                            <p className="hidden font-mono text-[11px] text-slate-500 sm:block">
                                    ...
                                    <div className="flex shrink-0 items-center gap-2">
                                        <Badge className="hidden gap-1.5 border-teal-500/20 bg-teal-500/10 font-mono text-xs text-teal-300 sm:flex">
                                            <Users className="h-3.5 w-3.5" />
                                            {visitorLabel}
                                        </Badge>
                                        {countries.length > 0 && (
                                            <Badge className="hidden gap-1.5 border-sky-500/20 bg-sky-500/10 font-mono text-xs text-sky-300 md:flex">
                                        ...
                                        <Button
                                            onClick={() => setIsOpen(false)}
                                            className="h-8 w-8 rounded-lg bg-white/5 p-0 text-slate-400 transition-colors hover:bg-white/10 hover:text-white"
                                            title="Close (ESC)"
                                            aria-label="Close"
                                        >
                                            <X className="h-4 w-4" />
                                ...
                                {/* ── Period filter ── */}
                                <div className="scrollbar-thin flex shrink-0 items-center gap-2 overflow-x-auto border-b border-white/[0.06] px-4 py-2">
                                    <span className="mr-1 shrink-0 font-mono text-[10px] uppercase tracking-[0.18em] text-slate-600">
                                        Period
                                    </span>
                                    {PERIODS.map((opt) => (
                                        <button
                                    ...
                                    {period === 'custom' && (
                                        <div className="ml-2 flex items-center gap-2">
                                            <input
                                                ...
                                                className="h-7 rounded-md border border-slate-700 bg-slate-900 px-2 font-mono text-[11px] text-slate-300 focus:border-teal-500 focus:outline-none"
                                                aria-label="From date"
                                            />
                                            <span className="font-mono text-[11px] text-slate-600">to</span>
                                            <input
                                                type="date"
                                                value={customTo}
                                            ...
                                            <button
                                                ...
                                                onClick={applyCustom}
                                                disabled={!customFrom}
                                                className="shrink-0 rounded-full border border-teal-500/30 bg-teal-500/15 px-3 py-1 font-mono text-[11px] text-teal-300 transition-all hover:bg-teal-500/25 disabled:cursor-not-allowed disabled:opacity-40"
                                            >
                                                Apply
                                            </button>
                                        </div>
                                ...
                                <div className="relative min-h-0 w-full flex-1">
                                    {loading ? (
                                        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4">
                                            ...
                                            >
                                                <Globe className="h-10 w-10 text-teal-500/50" />
                                            </motion.div>
                                        ...
                                        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3">
                                            <MapPin className="h-9 w-9 text-slate-700" />
                                            <p className="font-mono text-[11px] text-slate-600">
                                        ...
                                        <div className="scrollbar-thin flex max-h-[72px] flex-wrap justify-center gap-1.5 overflow-y-auto">
                                            {countries.map((c) => (
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.04] px-2.5 py-1 font-mono text-[11px] text-slate-300 transition-all hover:border-teal-500/30 hover:bg-white/[0.09] focus:outline-none focus-visible:ring-1 focus-visible:ring-teal-500/40"
                                                    title={`Focus ${c.name}`}
                                                >
                                                    <span aria-hidden>{c.flag}</span>
                                                    <span className="truncate">{c.name}</span>
                                                    <span className="shrink-0 text-slate-600">{c.count}</span>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
