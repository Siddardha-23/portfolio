...
function ScoreBar({ label, score, color }: { label: string; score: number; color?: string }) {
  ...
  const scoreColor = score >= 80 ? 'text-emerald-500 dark:text-emerald-400' : score >= 60 ? 'text-amber-500 dark:text-amber-400' : 'text-red-500 dark:text-red-400';
  const badge =
    score >= 90 ? { text: 'Excellent', cls: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/25' }
    : score >= 80 ? { text: 'Strong',   cls: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' }
    : score >= 70 ? { text: 'Good',     cls: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20' }
    : score >= 50 ? { text: 'Needs work', cls: 'bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/25' }
    : { text: 'Weak', cls: 'bg-red-500/15 text-red-600 dark:text-red-400 border-red-500/25' };
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center gap-2">
        <span className="text-xs text-gray-700 dark:text-gray-300 font-medium">{label}</span>
        <div className="flex items-center gap-1.5">
          <span className={`text-xs font-semibold tabular-nums ${scoreColor}`}>{score}</span>
          <span className={`hidden sm:inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wider border ${badge.cls}`}>{badge.text}</span>
        </div>
      </div>
  ...
  return (
    <button
      ...
      className={`opacity-0 group-hover:opacity-100 focus:opacity-100 transition-all inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[10px] font-medium ${
        copied ? 'text-emerald-500 bg-emerald-500/10' : 'text-gray-500 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-purple-500/10'
      }`}
...
function ResumePreview({ resume }: { resume: TailoredFullResume }) {
  const ST = ({ title, copyText }: { title: string; copyText?: () => string }) => (
    <div className="flex items-center gap-3 mb-3 group">
      <h3 className="text-xs font-bold uppercase tracking-widest text-purple-500 dark:text-purple-400/80">{title}</h3>
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden shadow-sm">
      <div className="bg-gradient-to-r from-purple-50 via-white to-purple-50 dark:from-purple-500/10 dark:via-gray-900 dark:to-purple-500/10 px-6 py-5 border-b border-gray-200 dark:border-gray-800 group relative">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">{resume.contact?.name}</h2>
        <p className="text-[12px] text-gray-600 dark:text-gray-300 mt-1">{[resume.contact?.phone, resume.contact?.email, resume.contact?.linkedin, resume.contact?.github].filter(Boolean).join('  ·  ')}</p>
        <div className="absolute top-4 right-4">
          <CopySectionButton getText={() => [resume.contact?.name, [resume.contact?.phone, resume.contact?.email, resume.contact?.linkedin, resume.contact?.github].filter(Boolean).join(' · ')].filter(Boolean).join('\n')} />
        </div>
      </div>
      <div className="px-6 py-5 space-y-5">
        {resume.summary && <div className="group"><ST title="Summary" copyText={() => resume.summary || ''} /><p className="text-[13.5px] text-gray-800 dark:text-gray-100 leading-[1.7]">{resume.summary}</p></div>}
        {resume.experience?.length > 0 && <div className="group"><ST title="Experience" copyText={() => (resume.experience || []).map(e => `${e.company}${e.location ? `, ${e.location}` : ''}${e.title ? ` — ${e.title}` : ''}${e.dates ? ` (${e.dates})` : ''}\n${(e.bullets || []).map(b => `  • ${b}`).join('\n')}`).join('\n\n')} /><div className="space-y-4">{resume.experience.map((exp, i) => (<div key={i}><div className="flex justify-between items-start gap-2"><div><p className="text-[13.5px] font-semibold text-gray-900 dark:text-white">{exp.company}{exp.location ? `, ${exp.location}` : ''}</p>{exp.title && <p className="text-[12px] text-gray-600 dark:text-gray-300 italic mt-0.5">{exp.title}</p>}</div><span className="text-[11px] text-gray-500 dark:text-gray-400 whitespace-nowrap shrink-0">{exp.dates}</span></div><ul className="mt-1.5 space-y-1">{exp.bullets?.map((b, j) => (<li key={j} className="text-[12.5px] text-gray-700 dark:text-gray-200 flex gap-2 leading-[1.65]"><span className="text-purple-500 dark:text-purple-400 shrink-0 mt-0.5">&bull;</span><span>{b}</span></li>))}</ul></div>))}</div></div>}
        {resume.projects?.length > 0 && <div className="group"><ST title="Projects" copyText={() => (resume.projects || []).map(p => `${p.name}\n${(p.bullets || []).map(b => `  • ${b}`).join('\n')}`).join('\n\n')} /><div className="space-y-3">{resume.projects.map((p, i) => (<div key={i}><p className="text-[13.5px] font-semibold text-gray-900 dark:text-white">{p.name}</p><ul className="mt-1 space-y-1">{p.bullets?.map((b, j) => (<li key={j} className="text-[12.5px] text-gray-700 dark:text-gray-200 flex gap-2 leading-[1.65]"><span className="text-purple-500 dark:text-purple-400 shrink-0 mt-0.5">&bull;</span><span>{b}</span></li>))}</ul></div>))}</div></div>}
        {resume.skills && Object.keys(resume.skills).length > 0 && <div className="group"><ST title="Technical Skills" copyText={() => Object.entries(resume.skills || {}).map(([c, s]) => `${c}: ${Array.isArray(s) ? s.join(', ') : String(s)}`).join('\n')} /><div className="space-y-2">{Object.entries(resume.skills).map(([c, s]) => (<div key={c} className="text-[12.5px] leading-[1.65]"><span className="font-semibold text-gray-900 dark:text-white">{c}: </span><span className="text-gray-700 dark:text-gray-200">{Array.isArray(s) ? s.join(', ') : String(s)}</span></div>))}</div></div>}
        {resume.education?.length > 0 && <div className="group"><ST title="Education" copyText={() => (resume.education || []).map(e => `${e.institution}${e.degree ? ` — ${e.degree}` : ''}${e.dates ? ` (${e.dates})` : ''}`).join('\n')} /><div className="space-y-2">{resume.education.map((edu, i) => { const d = edu.degree ? [...new Set(edu.degree.split('|').map(s=>s.trim()).filter(s=>!/^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|\d{4})/i.test(s)).filter(Boolean))].join(' | ') : ''; return (<div key={i} className="flex justify-between items-start gap-2"><p className="text-[12.5px] text-gray-800 dark:text-gray-100"><span className="font-semibold text-gray-900 dark:text-white">{edu.institution}</span>{d ? `  — ${d}` : ''}</p><span className="text-[11px] text-gray-500 dark:text-gray-400 whitespace-nowrap shrink-0">{edu.dates}</span></div>);})}</div></div>}
        {resume.certifications?.length > 0 && <div className="group"><ST title="Certifications" copyText={() => (resume.certifications || []).join('\n')} /><div className="flex flex-wrap gap-2">{resume.certifications.map((c, i) => (<span key={i} className="px-2.5 py-1 rounded-md text-[11px] font-medium bg-sky-500/10 text-sky-600 dark:text-sky-300 border border-sky-500/20">{c}</span>))}</div></div>}
      </div>
    </div>
  );
}
...
function ResumeDiffView({ prev, curr }: { prev: TailoredFullResume; curr: TailoredFullResume }) {
  const ST = ({ title, n }: { title: string; n?: number }) => (
    <div className="flex items-center gap-3 mb-3">
      <h3 className="text-xs font-bold uppercase tracking-widest text-purple-500 dark:text-purple-400/80">{title}</h3>
      {n !== undefined && n > 0 && <span className="text-[10px] font-medium text-gray-500 dark:text-gray-500">{n} change{n === 1 ? '' : 's'}</span>}
      <div className="flex-1 h-px bg-gray-200 dark:bg-gray-800" />
    </div>
  );
  ...
  (prev.experience || []).forEach(e => (e.bullets || []).forEach(b => prevExpBullets.add(b.trim())));
  (curr.experience || []).forEach(e => (e.bullets || []).forEach(b => currExpBullets.add(b.trim())));
  const prevProjBullets = new Set<string>();
  const currProjBullets = new Set<string>();
  (prev.projects || []).forEach(p => (p.bullets || []).forEach(b => prevProjBullets.add(b.trim())));
  (curr.projects || []).forEach(p => (p.bullets || []).forEach(b => currProjBullets.add(b.trim())));

  const DiffBullet = ({ text, type }: { text: string; type: 'added' | 'removed' | 'same' }) => {
    ...
    const markerCls = type === 'added' ? 'text-emerald-500' : type === 'removed' ? 'text-red-500' : 'text-gray-400';
    return (
      <li className={`text-[12.5px] flex gap-2 leading-[1.65] pl-2 border-l-2 rounded-r ${cls}`}>
  ...
  for (const v of Object.values(prev.skills || {})) if (Array.isArray(v)) v.forEach(s => prevSkillFlat.add(s));
  for (const v of Object.values(curr.skills || {})) if (Array.isArray(v)) v.forEach(s => currSkillFlat.add(s));
  currSkillFlat.forEach(s => { if (!prevSkillFlat.has(s)) addedSkills.add(s); });
  prevSkillFlat.forEach(s => { if (!currSkillFlat.has(s)) removedSkills.add(s); });
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden shadow-sm">
      <div className="bg-gradient-to-r from-amber-50 via-white to-purple-50 dark:from-amber-500/10 dark:via-gray-900 dark:to-purple-500/10 px-6 py-4 border-b border-gray-200 dark:border-gray-800">
        <h2 className="text-sm font-bold text-gray-900 dark:text-white">Diff view</h2>
        <p className="text-[11px] text-gray-600 dark:text-gray-400 mt-0.5">Changes between the previous and current tailored version</p>
        <div className="flex items-center gap-4 mt-2 text-[10px]">
          <span className="inline-flex items-center gap-1.5 text-emerald-600 dark:text-emerald-400"><span className="w-2 h-2 rounded-sm bg-emerald-500" />Added</span>
          <span className="inline-flex items-center gap-1.5 text-red-500 dark:text-red-400"><span className="w-2 h-2 rounded-sm bg-red-500" />Removed</span>
          <span className="inline-flex items-center gap-1.5 text-gray-500"><span className="w-2 h-2 rounded-sm bg-gray-400" />Unchanged</span>
        </div>
      </div>
      ...
      <div className="px-6 py-5 space-y-5">
        ...
        {(prevExpBullets.size > 0 || currExpBullets.size > 0) && (() => {
          const addedExp = [...currExpBullets].filter(b => !prevExpBullets.has(b));
          const removedExp = [...prevExpBullets].filter(b => !currExpBullets.has(b));
          ...
          return (
            <div>
              ...
              <ul className="space-y-1.5">
                {removedExp.map((b, i) => <DiffBullet key={`r-${i}`} text={b} type="removed" />)}
                {addedExp.map((b, i) => <DiffBullet key={`a-${i}`} text={b} type="added" />)}
              </ul>
            </div>
          );
        })()}
        ...
        {(prevProjBullets.size > 0 || currProjBullets.size > 0) && (() => {
          const addedProj = [...currProjBullets].filter(b => !prevProjBullets.has(b));
          const removedProj = [...prevProjBullets].filter(b => !currProjBullets.has(b));
          ...
          return (
            <div>
              ...
              <ul className="space-y-1.5">
                {removedProj.map((b, i) => <DiffBullet key={`r-${i}`} text={b} type="removed" />)}
                {addedProj.map((b, i) => <DiffBullet key={`a-${i}`} text={b} type="added" />)}
              </ul>
            </div>
          );
        })()}
        ...
        {(addedSkills.size > 0 || removedSkills.size > 0) && (
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {[...removedSkills].map(s => (
                <span key={`r-${s}`} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-red-500/10 text-red-500 dark:text-red-300 border border-red-500/25 line-through opacity-75">
                  <span className="font-bold">−</span>{s}
                </span>
              ))}
              {[...addedSkills].map(s => (
                <span key={`a-${s}`} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 border border-emerald-500/25">
                  <span className="font-bold">+</span>{s}
                </span>
              ))}
            </div>
        ...
        {/* Empty state */}
        {!summaryChanged && prevExpBullets.size === currExpBullets.size && [...prevExpBullets].every(b => currExpBullets.has(b)) && prevProjBullets.size === currProjBullets.size && [...prevProjBullets].every(b => currProjBullets.has(b)) && addedSkills.size === 0 && removedSkills.size === 0 && (
          <div className="text-center py-8">
            <p className="text-sm text-gray-500 dark:text-gray-400">No detectable changes between versions.</p>
          </div>
        )}
      </div>
    </div>
          ...
          return (
            <li key={step.key} className="flex items-start gap-3">
              <div className={`shrink-0 mt-0.5 w-5 h-5 rounded-full flex items-center justify-center transition-all duration-300 ${
                ...
                : status === 'active' ? 'bg-purple-500/15 text-purple-400 ring-1 ring-purple-500/40'
  ...
  return (
    <div className="space-y-6 sm:space-y-7">
      <div className="text-center pt-2 pb-1">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/10 border border-purple-500/20 mb-3"><SparklesIcon className="w-3.5 h-3.5 text-purple-400" /><span className="text-xs font-medium text-purple-500 dark:text-purple-300">AI-Powered Resume Tailoring</span></div>
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-3 leading-tight">Land more interviews with a<br /><span className="bg-gradient-to-r from-purple-500 to-indigo-500 dark:from-purple-400 dark:to-indigo-400 bg-clip-text text-transparent">perfectly tailored resume</span></h2>
          ...
          {[
            ...
            <div key={s.n} className="flex items-start sm:items-start gap-4 sm:gap-6 sm:flex-row">
              ...
              <div className="flex sm:flex-col items-center sm:text-center flex-1 min-w-0 gap-3 sm:gap-0">
                <div className={`w-11 h-11 rounded-full flex items-center justify-center sm:mb-2.5 shrink-0 ${s.active ? 'bg-gradient-to-br from-purple-500 to-purple-600 shadow-lg shadow-purple-500/25 text-white' : 'bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400'}`}>{s.icon}</div>
      ...
      <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files[0]; if (f) handleUpload(f); }}
        ...
        <div className="flex flex-col items-center text-center py-12 px-6">
          <div className="w-16 h-16 rounded-2xl bg-purple-500/10 flex items-center justify-center mb-4"><UploadCloudIcon className="w-8 h-8 text-purple-400" /></div>
          ...
          {uploading ? (
            <div className="w-full max-w-xs space-y-2"><div className="h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full animate-pulse" style={{ width: '60%' }} /></div><p className="text-xs text-gray-600 dark:text-gray-400">Parsing your resume...</p></div>
          ) : (
            <label className="cursor-pointer"><span className="inline-flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-medium text-white bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 transition-all duration-200"><UploadCloudIcon className="w-4 h-4" />Choose File</span><input type="file" accept=".pdf,.docx" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) handleUpload(f); }} /></label>
          )}
    ...
    <div className="space-y-5">
      ...
      {resumes.length === 0 ? (
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-white/[0.07] bg-white/60 dark:bg-gray-900/40 backdrop-blur-sm shadow-xl shadow-black/5 dark:shadow-black/20 divide-y divide-gray-200 dark:divide-white/[0.07] overflow-hidden">
          {resumes.map(r => (
            <div key={r.s3_key} className="flex items-center justify-between px-5 py-3.5 hover:bg-gray-100/60 dark:hover:bg-white/[0.03] transition-colors">
      ...
      {!hasContent ? (
        ...
        <div className="space-y-3">
          ...
          {records.map((r, idx) => {
            ...
            return (
              <div key={r.record_id || idx} className="rounded-2xl border border-gray-200 dark:border-white/[0.07] bg-white/60 dark:bg-gray-900/40 backdrop-blur-sm shadow-xl shadow-black/5 dark:shadow-black/20 overflow-hidden">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {Object.values(r.tailored_resume.skills).flat().slice(0, 15).map((s: any, si: number) => (
                            <span key={si} className="px-2 py-0.5 rounded text-[10px] font-medium bg-purple-500/10 text-purple-600 dark:text-purple-300 border border-purple-500/15">{s}</span>
                          ))}
          ...
          {generatedFiles.map(r => (
            <div key={r.s3_key} className="flex items-center justify-between px-5 py-3.5 rounded-2xl border border-gray-200 dark:border-white/[0.07] bg-white/60 dark:bg-gray-900/40 backdrop-blur-sm shadow-xl shadow-black/5 dark:shadow-black/20 hover:bg-gray-100 dark:hover:bg-gray-800/20 transition-colors">
              <div className="flex items-center gap-3 min-w-0 flex-1">
...
function ProfileTab() {
  ...
  const selectCls = 'w-full px-4 py-2.5 rounded-lg bg-gray-100 dark:bg-gray-800/60 border border-gray-300 dark:border-gray-700/60 text-sm text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-500/30 focus:border-purple-500/30 transition-all appearance-none cursor-pointer';
...
function TailorTab() {
  ...
  const [regenView, setRegenView] = useState<'regenerated' | 'previous' | 'diff'>('regenerated');
  ...
  const [versions, setVersions] = useState<{ id: string; resume: TailoredFullResume; label: string; timestamp: number }[]>([]);
  ...
  const handleTailoring = useCallback(async () => {
    ...
    // Phase 1: Analyze JD
    setAnalyzingJD(true); setTailorError(''); setResult(null);
    setTailorElapsed(0);
    ...
    setTimeout(() => progressRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 80);
  ...
  const handleRegenerate = useCallback(async () => {
    ...
    if (resp.data?.tailored_resume) {
      ...
      setVersions(v => [
        { id: `v${v.length + 1}-${Date.now()}`, resume: resp.data!.tailored_resume, label: feedbackSnippet || `Regeneration ${v.length + 1}`, timestamp: Date.now() },
        ...v,
      ]);
      setRegenFeedback('');
      setRegenView('regenerated');
      setInspectorTab('ats');
      toast.success('Resume regenerated — previous version kept for comparison');
  ...
  const handleRestoreVersion = useCallback((version: { resume: TailoredFullResume; label: string }) => {
    setResult(prev => prev ? { ...prev, tailored_resume: version.resume, ats_scores: undefined } : prev);
  ...
  const handleDownloadCoverLetter = useCallback(async () => {
    ...
    const r = await apiService.downloadCoverLetterPDF(
      ...
      result.jd_analysis.company && result.jd_analysis.company !== 'Not specified' ? result.jd_analysis.company : '',
    );
    setCoverLetterDownloading(false);
    if (r.error) { toast.error('Download failed', { description: r.error }); return; }
    if (r.data) {
      const u = URL.createObjectURL(r.data);
      const a = document.createElement('a'); a.href = u; a.download = r.filename || 'cover_letter.pdf'; a.click(); URL.revokeObjectURL(u);
  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-white/[0.07] bg-white/60 dark:bg-gray-900/40 backdrop-blur-sm px-4 sm:px-8 py-4 shadow-sm">
        <div className="flex items-center justify-center gap-2 sm:gap-4">
          {[
            ...
            const status = currentStep > s.n ? 'done' : currentStep === s.n ? 'active' : 'upcoming';
            return (
              <Fragment key={s.n}>
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all duration-300 ${
                    status === 'done' ? 'bg-gradient-to-br from-purple-500 to-indigo-600 text-white shadow-md shadow-purple-500/30'
                    : status === 'active' ? 'bg-gradient-to-br from-purple-500 to-indigo-600 text-white ring-4 ring-purple-500/20 shadow-lg shadow-purple-500/30'
                    : 'bg-gray-200 dark:bg-gray-800 text-gray-400 dark:text-gray-500'
                  }`}>
                    {status === 'done' ? (<svg className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.704 5.29a1 1 0 010 1.42l-7.5 7.5a1 1 0 01-1.42 0l-3.5-3.5a1 1 0 111.42-1.42L8.5 12.08l6.79-6.79a1 1 0 011.414 0z" clipRule="evenodd" /></svg>) : s.n}
                  </div>
                  <span className={`text-xs sm:text-sm font-medium hidden sm:inline transition-colors ${
                    ...
                    : status === 'done' ? 'text-purple-600 dark:text-purple-300'
                    : 'text-gray-400 dark:text-gray-500'
                  }`}>{s.label}</span>
                </div>
                {i < arr.length - 1 && (
                  <div className={`h-[2px] flex-1 max-w-[80px] rounded-full transition-colors duration-500 ${currentStep > s.n ? 'bg-gradient-to-r from-purple-500 to-indigo-500' : 'bg-gray-200 dark:bg-gray-800'}`} />
                )}
              </Fragment>
            );
          })}
      ...
      {result && (() => {
        type Nudge = { id: string; icon: string; text: React.ReactNode; actionLabel?: string; onAction?: () => void };
        const nudges: Nudge[] = [];
        ...
        if (versions.length >= 2 && result.ats_scores) {
          nudges.push({
            ...
            icon: '✨',
            text: <>Resume regenerated — <strong className="text-gray-900 dark:text-white">ATS score is now {result.ats_scores.overall}/100</strong>. Download your updated version when ready.</>,
        ...
        if (result.ats_scores && result.ats_scores.skills_alignment < 70 && result.ats_scores.missing_keywords && result.ats_scores.missing_keywords.length > 0) {
          const top = result.ats_scores.missing_keywords.slice(0, 2);
          nudges.push({
            id: `skills-${result.ats_scores.overall}`,
            icon: '💡',
            text: <>Your Skills Alignment is <strong className="text-amber-500 dark:text-amber-400">{result.ats_scores.skills_alignment}</strong> — try adding <strong className="text-gray-900 dark:text-white">"{top.join('"')}"</strong>{top.length > 1 ? ' and ' : ''}<strong className="text-gray-900 dark:text-white">"{top[top.length - 1]}"</strong> to bump it.</>,
        ...
        if (!coverLetter && !coverLetterLoading) {
          nudges.push({
            id: 'cover-letter',
            icon: '📝',
            text: <>You haven't generated a cover letter yet. It takes about <strong className="text-gray-900 dark:text-white">15 seconds</strong>.</>,
            actionLabel: 'Generate',
            onAction: () => { setInspectorTab('cover'); setTimeout(handleCoverLetter, 300); },
          });
        }
        ...
        return (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.02] to-transparent" aria-hidden="true" />
            <div className="relative flex items-center gap-2.5 text-sm min-w-0">
              <span className="text-lg leading-none shrink-0" aria-hidden="true">{active.icon}</span>
              <p className="text-gray-700 dark:text-gray-300 leading-snug min-w-0">{active.text}</p>
            </div>
            <div className="relative flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={() => setDismissedNudges(d => new Set(d).add(active.id))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)] gap-5 items-start">
        ...
        <section className="min-w-0 lg:sticky lg:top-24 lg:self-start" aria-label="Resume preview">
          <div className="relative rounded-2xl border border-gray-200 dark:border-white/[0.07] bg-gray-50/40 dark:bg-gray-900/40 backdrop-blur-sm shadow-xl shadow-black/5 dark:shadow-black/30 overflow-hidden flex flex-col lg:max-h-[calc(100vh-140px)]">
            ...
            {result?.ats_scores && (() => {
              ...
              const color = s >= 80 ? 'from-emerald-500 to-emerald-400' : s >= 60 ? 'from-amber-500 to-amber-400' : 'from-red-500 to-red-400';
              return (
                <div className="absolute left-0 top-0 bottom-0 w-1 overflow-hidden z-10" aria-hidden="true">
                  <div
            ...
            <div className="shrink-0 px-5 py-3.5 border-b border-gray-200 dark:border-white/[0.07] flex items-center justify-between gap-3 bg-white/50 dark:bg-gray-900/60 flex-wrap">
              <div className="flex items-center gap-3 min-w-0">
                ...
                {previousResume && (
                  <div role="tablist" aria-label="Resume version" className="inline-flex rounded-lg bg-gray-200/70 dark:bg-gray-800/70 p-0.5 border border-gray-300 dark:border-white/10">
                    {([
                      { key: 'regenerated' as const, label: 'Tailored', active: 'bg-purple-500/20 text-purple-600 dark:text-purple-300 ring-1 ring-purple-500/30 shadow-sm' },
                      { key: 'previous' as const, label: 'Original', active: 'bg-white dark:bg-white/10 text-gray-900 dark:text-white ring-1 ring-gray-300 dark:ring-white/20 shadow-sm' },
                      { key: 'diff' as const, label: 'Diff', active: 'bg-amber-500/15 text-amber-600 dark:text-amber-300 ring-1 ring-amber-500/30 shadow-sm' },
                    ]).map(t => (
                      <button
                        key={t.key}
                        role="tab"
                        ...
                        className={`relative px-3 py-1 rounded-md text-[11px] font-semibold transition-all ${
                          regenView === t.key ? t.active : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
                        }`}
                      >
                        {t.label}
                      </button>
              ...
              <div className="flex items-center gap-1.5">
                {result && versions.length > 1 && (
                  <div className="relative">
                    ...
                    >
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                      History
                      <span className="inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full text-[10px] font-bold bg-purple-500/15 text-purple-600 dark:text-purple-300 border border-purple-500/25">{versions.length}</span>
                    </button>
                    {historyOpen && (
                      <>
                        ...
                        >
                          <div className="px-3 py-2 border-b border-gray-200 dark:border-white/[0.07] mb-1">
                            <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Version History</p>
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">Click any version to restore it</p>
                          </div>
                          <div className="max-h-80 overflow-y-auto">
                            {versions.map((v, idx) => {
                              const isCurrent = idx === 0;
                              const ageMin = Math.max(1, Math.floor((Date.now() - v.timestamp) / 60000));
                              return (
                                <button
                                  key={v.id}
                                ...
                                >
                                  <div className="flex items-center justify-between gap-2 mb-0.5">
                                    <span className={`text-xs font-semibold ${isCurrent ? 'text-purple-600 dark:text-purple-300' : 'text-gray-900 dark:text-white'}`}>
                                      v{versions.length - idx}{isCurrent && ' · current'}
                                    </span>
                                    <span className="text-[10px] text-gray-400 dark:text-gray-500 tabular-nums">
                                      {idx === versions.length - 1 ? 'uploaded' : `${ageMin}m ago`}
                                    </span>
                                  </div>
                                  <p className={`text-[11px] leading-snug truncate ${isCurrent ? 'text-purple-600/80 dark:text-purple-300/80' : 'text-gray-600 dark:text-gray-400'}`}>
                                    {v.label}
                                  </p>
                                </button>
                              );
                ...
                {result && previousResume && (
                  <button
                    onClick={() => { if (previousResume) { setResult(prev => prev ? { ...prev, tailored_resume: previousResume, ats_scores: undefined } : prev); setPreviousResume(null); setRegenView('regenerated'); toast.success('Reverted to previous version'); atsAutoTriggered.current = false; } }}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 hover:border-purple-500/40 hover:text-purple-600 dark:hover:text-purple-300 transition-all"
                  >
                    <ArrowPathIcon className="w-3.5 h-3.5" />Revert
                  </button>
                )}
              </div>
            </div>
            ...
            {result ? (
              <div className="p-3 sm:p-5 bg-gray-100/60 dark:bg-gray-950/80 space-y-3">
                ...
                {previousResume && (() => {
                  ...
                  for (const e of prev.experience || []) for (const b of e.bullets || []) prevBullets.add(b.trim());
                  for (const p of prev.projects || [])  for (const b of p.bullets || []) prevBullets.add(b.trim());
                  for (const e of curr.experience || []) for (const b of e.bullets || []) currBullets.add(b.trim());
                  for (const p of curr.projects || [])  for (const b of p.bullets || []) currBullets.add(b.trim());
                  const added = [...currBullets].filter(b => !prevBullets.has(b)).length;
                  const removed = [...prevBullets].filter(b => !currBullets.has(b)).length;
                  const edited = Math.min(added, removed); // best-effort heuristic
                  const prevSkills = new Set<string>();
                  const currSkills = new Set<string>();
                  for (const v of Object.values(prev.skills || {})) if (Array.isArray(v)) v.forEach(s => prevSkills.add(s.toLowerCase()));
                  for (const v of Object.values(curr.skills || {})) if (Array.isArray(v)) v.forEach(s => currSkills.add(s.toLowerCase()));
                  const keywordsAdded = [...currSkills].filter(s => !prevSkills.has(s)).length;
                  return (
                    <motion.div
                      initial={{ opacity: 0, y: -8 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-200">What Changed</p>
                        <span className="text-[10px] text-gray-500 dark:text-gray-400">vs previous version</span>
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        {[
                          { n: added,         label: 'bullets added',    cls: 'text-emerald-600 dark:text-emerald-400', sign: '+' },
                          { n: removed,       label: 'bullets removed',  cls: 'text-red-500 dark:text-red-400',         sign: '−' },
                          { n: edited,        label: 'bullets edited',   cls: 'text-amber-500 dark:text-amber-400',     sign: '~' },
                          { n: keywordsAdded, label: 'keywords added',   cls: 'text-purple-600 dark:text-purple-300',   sign: '+' },
                        ].map((stat, i) => (
                          <div key={i} className="rounded-lg bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.07] p-2.5">
                            <p className={`text-lg font-bold tabular-nums leading-none ${stat.cls}`}>{stat.sign}{stat.n}</p>
                            <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1">{stat.label}</p>
                          </div>
                        ))}
                      </div>
                    </motion.div>
                ...
                >
                  {previousResume && regenView === 'diff' ? (
                    <ResumeDiffView prev={previousResume} curr={result.tailored_resume} />
              ...
              >
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[15px] font-semibold text-gray-900 dark:text-white">Tailoring your resume</p>
                      <p className="text-[11px] text-gray-500 dark:text-gray-400">{tailorElapsed >= 90 ? 'Taking longer than expected' : 'Usually takes 30–60 seconds'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {tailorElapsed > 0 && <span className="text-[11px] text-gray-400 dark:text-gray-500 tabular-nums font-medium">{tailorElapsed}s</span>}
                    {tailorElapsed >= 90 && (
                      <button onClick={handleCancel} className="px-2.5 py-1 text-[11px] font-medium text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-white/10 rounded-md hover:border-red-400/60 hover:text-red-400 transition-all">Cancel</button>
                    )}
                  </div>
                </div>
                ...
                <ol className="space-y-3 flex-1">
                  {[
                    ...
                    const current = analyzingJD ? 0 : !tailoring ? -1 : tailorElapsed < 15 ? 1 : tailorElapsed < 40 ? 2 : 3;
                    const status = i < current ? 'done' : i === current ? 'active' : 'pending';
                    return (
                      <li key={step.key} className="flex items-start gap-3">
                        <div className={`shrink-0 mt-0.5 w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 ${
                          status === 'done' ? 'bg-emerald-500/15 text-emerald-400 ring-1 ring-emerald-500/40'
                          : status === 'active' ? 'bg-purple-500/15 text-purple-400 ring-2 ring-purple-500/40 animate-pulse'
                          : 'bg-gray-200 dark:bg-gray-800/60 text-gray-400 dark:text-gray-600 ring-1 ring-gray-300 dark:ring-white/10'
                        }`}>
                          {status === 'done' ? (
                            <svg className="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.704 5.29a1 1 0 010 1.42l-7.5 7.5a1 1 0 01-1.42 0l-3.5-3.5a1 1 0 111.42-1.42L8.5 12.08l6.79-6.79a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                          ) : status === 'active' ? (
                        ...
                        <div className="min-w-0 flex-1 pt-0.5">
                          ...
                          {status === 'active' && (
                            <>
                              <p className="text-[12px] text-gray-500 dark:text-gray-400 mt-0.5">{step.hint}</p>
              ...
              >
                ...
                <p className="text-base font-semibold text-gray-900 dark:text-white mb-1">Ready to tailor</p>
                <p className="text-sm text-gray-600 dark:text-gray-400 max-w-xs leading-relaxed">Paste a job description in the <span className="text-purple-600 dark:text-purple-400 font-medium">Job Description</span> panel on the right and click <span className="text-purple-600 dark:text-purple-400 font-medium">Tailor My Resume</span> to begin.</p>
                <div className="mt-6 flex items-center gap-2 text-[11px] text-gray-500 dark:text-gray-500">
                  <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
        ...
        <section className="min-w-0 flex flex-col" aria-label="Inspector">
          {/* Tab bar */}
          <div role="tablist" className="flex gap-1 bg-gray-100/80 dark:bg-gray-900/60 backdrop-blur-sm rounded-2xl p-1.5 border border-gray-200 dark:border-white/[0.07] mb-4 overflow-x-auto hide-scrollbar">
            {([
              { key: 'jd' as const,         label: 'Job Description', icon: <ClipboardIcon className="w-4 h-4" />,         disabled: false },
              { key: 'ats' as const,        label: 'ATS Score',       icon: <MagnifyingGlassIcon className="w-4 h-4" />,  disabled: !result },
              { key: 'regenerate' as const, label: 'Regenerate',      icon: <ArrowPathIcon className="w-4 h-4" />,         disabled: !result },
              { key: 'cover' as const,      label: 'Cover Letter',    icon: <SparklesIcon className="w-4 h-4" />,          disabled: !result },
            ]).map(t => {
              ...
              {inspectorTab === 'jd' && (
                <div className="p-5 sm:p-6 space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <h2 className="text-base font-semibold text-gray-900 dark:text-white">Job Description</h2>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Paste the complete job posting — we'll extract requirements automatically</p>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={async () => { try { const t = await navigator.clipboard.readText(); if (t) setJdText(t.slice(0, 10000)); } catch { toast.error('Clipboard access denied'); } }}
                      ...
                      {jdText && <button onClick={() => setJdText('')} disabled={analyzingJD || tailoring} className="text-[11px] text-gray-500 dark:text-gray-400 hover:text-red-400 transition-colors px-2 py-1.5 disabled:opacity-50">Clear</button>}
                    </div>
                  </div>
                      ...
                      <div className="relative pt-3 mt-1">
                        ...
                        <div className="rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/[0.07] p-4 space-y-4">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-[10px] uppercase tracking-wider font-semibold text-gray-600 dark:text-gray-300">Extracted Requirements</span>
                            <span className="text-[10px] text-gray-400 dark:text-gray-500">— auto-detected from job posting</span>
                          </div>

                          <div>
                            <p className="text-[11px] font-medium text-gray-500 dark:text-gray-500 mb-1">Role</p>
                            <p className="text-sm font-medium text-gray-900 dark:text-white">
                              {result.jd_analysis.job_title}
                              {result.jd_analysis.company && result.jd_analysis.company !== 'Not specified' && <> at <span className="text-gray-700 dark:text-gray-300">{result.jd_analysis.company}</span></>}
                            </p>
                          </div>

                          {result.jd_analysis.required_skills?.length > 0 && (() => {
                            const skills = result.jd_analysis.required_skills.slice(0, 14);
                            const matchedCount = skills.filter(isMatched).length;
                            return (
                              <div>
                                <div className="flex items-center justify-between mb-2">
                                  <p className="text-[11px] font-medium text-gray-500 dark:text-gray-500">Required skills</p>
                                  <span className="text-[10px] font-semibold tabular-nums text-gray-500 dark:text-gray-400">{matchedCount}/{skills.length} matched</span>
                                </div>
                                <div className="flex flex-wrap gap-1.5">
                                  {skills.map(s => {
                                  ...
                                  {result.jd_analysis.required_skills.length > 14 && (
                                    <span className="text-[10px] text-gray-500 dark:text-gray-500 self-center">+{result.jd_analysis.required_skills.length - 14} more</span>
                                  )}
                                </div>
                              </div>
                            );
                          ...
                          {result.jd_analysis.preferred_skills?.length > 0 && (
                            <div>
                              <p className="text-[11px] font-medium text-gray-500 dark:text-gray-500 mb-2">Preferred skills</p>
                              <div className="flex flex-wrap gap-1.5">
                                {result.jd_analysis.preferred_skills.slice(0, 10).map(s => {
                                  const matched = isMatched(s);
                                  return (
                                    <span key={s}
                                    ...
                                    >
                                      {matched && <svg className="w-2.5 h-2.5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.704 5.29a1 1 0 010 1.42l-7.5 7.5a1 1 0 01-1.42 0l-3.5-3.5a1 1 0 111.42-1.42L8.5 12.08l6.79-6.79a1 1 0 011.414 0z" clipRule="evenodd" /></svg>}
                                      {s}
                                    </span>
                                  );
                                })}
                  ...
                  >
                    {analyzingJD || tailoring ? (
                      <><span className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent" />Tailoring…</>
                    ) : (
                      <><SparklesIcon className="w-4 h-4" />{result ? 'Re-tailor with new JD' : 'Tailor My Resume'}</>
                    )}
                  </button>
                </div>
              )}
                ...
                <div className="p-5 sm:p-6 lg:max-h-[calc(100vh-220px)] lg:overflow-y-auto overscroll-contain">
                  {result.ats_scores ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <p className="text-sm font-semibold text-gray-900 dark:text-white mb-1">Analyzing ATS compatibility…</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 max-w-xs">Checking against Workday, Greenhouse, Lever, and more</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {inspectorTab === 'regenerate' && result && (
                <div className="p-5 sm:p-6 space-y-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-base font-semibold text-gray-900 dark:text-white">Regenerate Resume</h2>
                      <p className="text-xs text-gray-500 dark:text-gray-400">Refine with feedback — your current version is kept for comparison</p>
                    </div>
                  </div>

                  {(() => {
                    const SUGGESTIONS: { tier: 'high' | 'style' | 'extra'; label: string }[] = [
                      { tier: 'high',  label: 'Quantify impact with numbers' },
                      { tier: 'high',  label: 'Add more metrics to bullets' },
                      { tier: 'high',  label: 'Highlight cloud / AWS experience' },
                      ...
                      { tier: 'extra', label: 'Emphasize leadership experience' },
                      { tier: 'extra', label: 'Add team / collaboration keywords' },
                    ];
                    const selected = SUGGESTIONS.filter(s => regenFeedback.toLowerCase().includes(s.label.toLowerCase().slice(0, 20)));

                    const chipCls = (tier: 'high' | 'style' | 'extra', already: boolean) => {
                      if (already) {
                      ...
                      if (tier === 'high')  return 'bg-purple-500/15 text-purple-600 dark:text-purple-300 border-purple-500/30 font-semibold hover:bg-purple-500/20 hover:border-purple-500/50 hover:-translate-y-px';
                      if (tier === 'style') return 'bg-gray-100 dark:bg-white/[0.05] text-gray-700 dark:text-gray-300 border-gray-200 dark:border-white/10 hover:border-purple-500/40 hover:text-purple-600 dark:hover:text-purple-300 hover:-translate-y-px';
                    ...
                    const Chip = ({ hint }: { hint: { tier: 'high' | 'style' | 'extra'; label: string } }) => {
                      const already = regenFeedback.toLowerCase().includes(hint.label.toLowerCase().slice(0, 20));
                      return (
                        <button
                          type="button"
                          disabled={already || regenerating}
                          onClick={() => setRegenFeedback(f => {
                            if (!f.trim()) return hint.label + '.';
                            if (f.toLowerCase().includes(hint.label.toLowerCase().slice(0, 20))) return f;
                            return f.trim().replace(/\.?$/, '.') + ' ' + hint.label + '.';
                          })}
                        ...
                        >
                          {already ? (
                            ...
                            <svg className="w-3 h-3" viewBox="0 0 20 20" fill="currentColor"><path d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" /></svg>
                          ) : (
                            <span className="text-[11px] leading-none">+</span>
                          )}
                          {hint.label}
                    ...
                    return (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Quick suggestions</p>
                          {selected.length > 0 && (
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {SUGGESTIONS.filter(s => s.tier === 'high').map(s => <Chip key={s.label} hint={s} />)}
                          </div>
                        </div>

                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 mb-1.5">Style</p>
                          <div className="flex flex-wrap gap-1.5">
                            {SUGGESTIONS.filter(s => s.tier === 'style').map(s => <Chip key={s.label} hint={s} />)}
                          </div>
                        </div>

                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-500 mb-1.5">Extras</p>
                          <div className="flex flex-wrap gap-1.5">
                            {SUGGESTIONS.filter(s => s.tier === 'extra').map(s => <Chip key={s.label} hint={s} />)}
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  <div>
                    <label className="text-[10px] font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2 block">Custom instructions</label>
                    <textarea
                  ...
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">Personalized to the role and your resume</p>
                      </div>
                    </div>
                    {!coverLetter && (
                      <button
                      ...
                      >
                        {coverLetterLoading ? (<><span className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent" />Generating…</>) : (<><SparklesIcon className="w-4 h-4" />Generate</>)}
                      </button>
                    )}
                  </div>
                  ...
                  {coverLetter ? (
                    <>
                      <div className="rounded-xl bg-white dark:bg-gray-950/80 border border-gray-200 dark:border-white/[0.07] p-5 sm:p-6 max-h-[480px] overflow-y-auto shadow-inner shadow-black/5">
                        <div className="text-[14px] text-gray-800 dark:text-gray-100 leading-[1.75] whitespace-pre-line font-serif">{coverLetter}</div>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <button onClick={handleDownloadCoverLetter} disabled={coverLetterDownloading}
                          ...
                          <DocumentArrowDownIcon className="w-4 h-4" />{coverLetterDownloading ? 'Preparing…' : 'Download PDF'}
                        </button>
                        ...
                        <button onClick={() => setCoverLetter(null)}
                    ...
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <p className="text-sm font-semibold text-gray-900 dark:text-white mb-1">Writing your cover letter…</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 max-w-xs">Matching your experience to the role. Usually takes 15–30 seconds.</p>
                    </div>
                  ) : (
                    <div className="space-y-3 py-4">
                      <div className="flex items-start gap-3 p-4 rounded-xl bg-purple-500/5 border border-purple-500/20">
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">We'll generate a personalized 3-4 paragraph cover letter tailored to <span className="font-semibold text-gray-900 dark:text-white">{result.jd_analysis.job_title}</span>{result.jd_analysis.company && result.jd_analysis.company !== 'Not specified' ? <> at <span className="font-semibold text-gray-900 dark:text-white">{result.jd_analysis.company}</span></> : null} using your tailored resume.</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 hidden sm:block">
                ...
                <p className="text-[11px] text-gray-500 dark:text-gray-400 truncate">
                  Tailored for <span className="text-purple-600 dark:text-purple-400 font-medium">{result.jd_analysis.job_title}</span>
                  {result.jd_analysis.company && result.jd_analysis.company !== 'Not specified' && <> at <span className="text-gray-700 dark:text-gray-300">{result.jd_analysis.company}</span></>}
              ...
              <div className="min-w-0 sm:hidden">
                <p className="text-[11px] font-semibold text-gray-900 dark:text-white truncate">Your Tailored Resume</p>
                <p className="text-[11px] text-purple-600 dark:text-purple-400 truncate">{result.jd_analysis.job_title}</p>
              </div>
            </div>

            {/* CENTER — ATS match badge (lg+) */}
            {result.ats_scores && (
              <div className={`hidden lg:flex items-center gap-2 px-4 py-2 rounded-xl border shrink-0 ${
                ...
                <div className={`w-2 h-2 rounded-full animate-pulse ${result.ats_scores.overall >= 80 ? 'bg-emerald-400' : result.ats_scores.overall >= 60 ? 'bg-amber-400' : 'bg-red-400'}`} />
                <span className={`text-xs font-semibold tabular-nums ${result.ats_scores.overall >= 80 ? 'text-emerald-600 dark:text-emerald-400' : result.ats_scores.overall >= 60 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <div className="relative">
                ...
                >
                  {downloading !== null ? (
                    <><span className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent" /><span className="hidden sm:inline">Preparing…</span></>
                  ) : (
                    <>
                      <DocumentArrowDownIcon className="w-4 h-4" />
                      <span className="hidden sm:inline">Download Resume</span>
                      <span className="sm:hidden">Download</span>
                      <svg className={`w-3.5 h-3.5 opacity-70 transition-transform ${downloadMenuOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                    </>
                  )}
                </button>
                {downloadMenuOpen && (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-gray-500 dark:text-gray-400">Best for job applications</p>
                        </div>
                      </button>
                      <button
                        role="menuitem"
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-gray-900 dark:text-white">Download as DOCX</p>
                          <p className="text-[11px] text-gray-500 dark:text-gray-400">Best for further editing</p>
                        </div>
                      </button>
                    </motion.div>
                  </>
...
export default function ResumeParser() {
  ...
  return (
    ...
    >
      <div className="min-h-screen bg-white dark:bg-gray-950 antialiased pb-24" style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
          ...
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="h-14 flex items-center gap-4">
              ...
              <div className="flex items-center gap-2.5 shrink-0">
                ...
                <h1 className="text-lg font-bold bg-gradient-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent hidden sm:block">Resume Tailor</h1>
              </div>

              {/* Nav pills — center-ish */}
              <nav className="flex-1 flex items-center gap-1 overflow-x-auto hide-scrollbar" aria-label="Resume tailor sections">
                {NAV_ITEMS.map(item => {
                  ...
                  return (
                    ...
                    >
                      <span className={active ? 'text-purple-500 dark:text-purple-300' : 'text-gray-400 dark:text-gray-500'}>{item.icon}</span>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 mt-2 w-56 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 shadow-2xl shadow-black/20 backdrop-blur-xl overflow-hidden z-20">
                        <div className="px-4 py-3 border-b border-gray-200 dark:border-white/10">
                          <p className="text-sm font-semibold text-gray-900 dark:text-white truncate">{user?.name || user?.email?.split('@')[0]}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{user?.email}</p>
                        </div>
                        <button onClick={() => { setActiveNav('profile'); setUserMenuOpen(false); }}
                          className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">Profile settings</button>
                      </div>
                    </>
                  )}
                </div>
        ...
        <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {activeNav === 'batch' && <Suspense fallback={<div className="animate-pulse h-48 rounded-2xl bg-gray-100 dark:bg-gray-800/40" />}><BatchTailor /></Suspense>}
